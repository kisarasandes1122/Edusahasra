...
import { FaMapMarkerAlt, FaTruck, FaWalking, FaBoxOpen, FaPencilAlt, FaMinus, FaPlus } from 'react-icons/fa';
import api from '../../../api';
...
const DonationPage = () => {
  ...
  const [donatedQuantities, setDonatedQuantities] = useState({});
    ...
    const fetchRequest = async () => {
      ...
      try {
        ...
        response.data.requestedItems.forEach(item => {
          initialQuantities[item.categoryId] = 0;
        });
  ...
  const handleSubmit = async (e) => {
    ...
    const donationData = {
      ...
      deliveryMethod: selectedDelivery === 'partner' ? 'Courier' : 'Self-Delivery',
    ...
    try {
      ...
      if (response.status === 201 && response.data.donation) {
          setCreatedDonationId(response.data.donation._id);
